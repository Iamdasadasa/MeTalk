...
struct TimeCalculator {
    ...
    static func isMoreThanOneMonthAgo(from date: Date) -> Bool {
        // 現在の日付
        let currentDate = Date()
        // 一ヶ月前の日付を計算
        if let oneMonthAgo = Calendar.current.date(byAdding: .month, value: -1, to: currentDate) {
            // 指定した日付が一ヶ月以上前かどうかを判断
            return date.compare(oneMonthAgo) == .orderedAscending
        } else {
            // 一ヶ月前の日付を計算できなかった場合は、例外的なケースとしてfalseを返す
            return false
        }
    }
...
///広告カウンターUserDefaults処理
class AdsCounterManager {
    
    static let shared = AdsCounterManager()
    
    private let userDefaultsKey = "counter"
    ...
    func incrementCounter() {
        ...
        currentCounter += 1
        UserDefaults.standard.set(currentCounter, forKey: userDefaultsKey)
    }
