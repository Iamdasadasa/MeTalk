...
class showUserListViewController:UIViewController,UINavigationControllerDelegate{
    ...
    private var interstitialAdsView:GADInterstitialAd?///インタースティシャル広告
    let adsCounterManager = AdsCounterManager.shared  ///広告表示までのカウント
    let defaults = UserDefaults.standard    ///端末保存インスタンス
...
extension showUserListViewController {
    ...
    func hostAndlocalProfileDataCreateAndRegister(roomID:String,CELL:UserListTableViewCell) {
        ...
        }, Message: message, text: "", roomID: roomID, Like: true, receiverID: targetProfile.Required_UID, senderNickname: self.SELFINFO.Required_NickName)
        DispatchQueue.main.async {
            ///広告表示
            self.interstitialAdsShowing()
...
extension showUserListViewController {
    ...
    func ApiKeyGetter(callback: @escaping (String,String,String) -> Void) {
        ...
        if let APIKEYOBJECT = APIKEYLOCALGETTER.getter(),
            let APPID = APIKEYOBJECT.appID,let APIKey = APIKEYOBJECT.APIKey
            {
                if APIKey != "ERROR" {
                    callback(APPID,APIKey,APIKEYOBJECT.version!)
                    return
                }
        }
...
///広告の実装
extension showUserListViewController:GADFullScreenContentDelegate {
    ///バナー広告
    ...
    ///インタースティシャル広告の準備
    func interstitialAdsSetting() {

        let request = GADRequest()
        GADInterstitialAd.load(withAdUnitID:ADSInfoSingleton.shared.interstitialAdUnitID,
                                    request: request,
                          completionHandler: { [self] ad, error in
                            ...
                            interstitialAdsView = ad
                            interstitialAdsView?.fullScreenContentDelegate = self
                          }
        )
    }
    ...
    //インタースティシャル広告の表示
    func interstitialAdsShowing() {
        ///管理者の場合は表示しない
        if defaults.bool(forKey: "admin") {
            return
        }
        ///ライク数をインクリメント
        adsCounterManager.incrementCounter()
        ///ライク数を取得
        let pushCount = adsCounterManager.getCounter()
        ///ライク数5未満の場合何もしない
        if pushCount < 5 {
            return
        ///10を上回っていたら表示
        } else if pushCount > 10 {
            if interstitialAdsView != nil {
                interstitialAdsView?.present(fromRootViewController: self)
                adsCounterManager.resetCounter()
            } else {
                interstitialAdsSetting()
            }
        }
